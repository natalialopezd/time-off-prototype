...
export const ModuleSelectionPopover = ({ selectedUserIds }: { selectedUserIds: Set<number> }) => {
  ...
  const message = useMemo(() => {
    ...
    if (isChecked && usersGaining > 0) {
      ...
      if (isPaid) return `${base} \u2022 ${projectedSeats}/${totalSeats} seats used`
    }
    ...
    return null
  }, [isIndeterminate, usersWithAccess, selectedIds.length, hasChanges, isChecked, checkState, usersGaining, usersLosing, isTrial, isPaid, projectedSeats, totalSeats])
    ...
    <Dropdown
      trigger={
        <Button variant="secondary">
          <IconTimeOff size={14} className="text-secondary" />
          Modules
        </Button>
      }
      open={open}
    ...
    >
      <div>
        <div className="flex justify-between items-center px-3 py-2">
          <span className="text-h6 font-semibold text-secondary uppercase">Modules</span>
        ...
        >
          ...
          <div className="flex size-6 items-center justify-center rounded-lg bg-tertiary shrink-0">
            <IconTimeOff size={14} className="text-secondary" />
          </div>
          <span className="flex-1 truncate text-p1 text-primary text-left">Time Off</span>
        </button>

        {message && (
          <>
            <div className="mx-3 border-t border-primary" />
            <p className="text-p2 text-secondary px-3 py-2">{message}</p>
          </>
        )}

        <div className="p-3 flex justify-end border-t border-secondary">
